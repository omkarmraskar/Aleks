class Snapping {
  // Function to snap a symbol to the closest endpoint of a line in a graph
  snapSymbol(newNode) {
    const edges = graph.getRecall().edges;
    const newNodex = Number(newNode.x);
    const newNodey = Number(newNode.y);
    ...
    for (let i = 0; i < edges.length; i++) {
      ...
      const dist =
        Math.abs((y2 - y1) * x - (x2 - x1) * y + x2 * y1 - y2 * x1) / length;
      ...
      if (dist <= 10) {
        ...
        const dist1 = Math.sqrt(
          (x - candidate1[0]) ** 2 + (y - candidate1[1]) ** 2
        );
        const dist2 = Math.sqrt(
          (x - candidate2[0]) ** 2 + (y - candidate2[1]) ** 2
        );
    ...
    newNode.x = Math.floor(x);
    newNode.y = Math.floor(y);
  }
}
